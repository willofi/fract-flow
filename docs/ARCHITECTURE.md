...
이 문서는 FractFlow의 기술적 중추를 구성하는 상태 관리, 백엔드 통합 및 동기화 로직에 대해 설명합니다.

---

## 🏗 시스템 개요

FractFlow는 UI 응답성과 데이터 영속성 간의 균형을 맞추기 위해 **"Tri-State"** 동기화 모델을 사용합니다:
...
Next.js 15+ App Router 환경에서 `next-intl`을 사용하여 정교한 다국어 아키텍처를 구현했습니다.

### 1. URL 기반 로케일 라우팅
모든 페이지는 `src/app/[locale]` 구조를 따르며, URL에 로케일(예: `/ko`, `/en`)이 명시됩니다. 이는 SEO 최적화와 공유 시 언어 일관성을 보장합니다.

### 2. 하이브리드 프록시 시스템
`src/proxy.ts`는 두 가지 핵심 기능을 수행합니다:
- **로케일 감지**: 사용자의 브라우저 설정을 감지하여 최적의 언어로 자동 리다이렉트합니다.
- **인증 세션 관리**: Supabase Auth와 통합되어, 다국어 라우팅 중에도 사용자 세션 정보를 안전하게 유지합니다.
...
- **next-intl**: 전역 다국어 컨텍스트를 제공하며, 서버/클라이언트 컴포넌트 간의 매끄러운 번역 브릿지 역할을 합니다.
